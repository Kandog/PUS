...
EXT_DIR=/home/oracle/scripts/Interface/Paymentus/AMBP/exttable
EXT_FILE=${EXT_DIR}/paymentus_ambp_file
...
${EXECUTE_DIR}/paymentus_ambp_returns.ksh


# Remove old log file
if [ -f ${LOG_DIR}/paymentus_ambp.log ]; then
  rm ${LOG_DIR}/paymentus_ambp.log
fi
exec >>${LOG_DIR}/paymentus_ambp.log
exec 2>&1
...
if [ -z "$(ls -A ${IN_DIR}|head -n 5)" ];then
  echo "$(date) - No Buildin Permit file get from Paymentus." <${LOG_DIR}/Error.log
#  mailx -s "No Building Permit file get from Paymentus" -r ${FROM_ADDR} ${TO_ADDR} <${LOG_DIR}/Error.log
  # Audit
  echo "$(date) - No any Building Permit file get from Paymentus.">>${AUDIT_LOG}
  exit
fi
...
for f in ${IN_DIR}/*;do
  if [ -f "$f" ];then
    ...
    if [ $? != 0 ];then
        ...
        # process data
        sqlplus /@AMAT1 @${EXECUTE_DIR}/paymentus_ambp.sql >  ${LOG_DIR}/sql.log
